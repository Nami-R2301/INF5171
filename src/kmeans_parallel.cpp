...
void KMeansParallel::fit(const std::vector<Point>& points, const std::vector<Point>& start_points) {
  ...
  while (true) {
    ...
    size_t result = tbb::parallel_reduce(
        tbb::blocked_range<size_t>(0, points.size()), 0,
        [&](const tbb::blocked_range<size_t>& range, size_t changes) {
          for (size_t i = range.begin(); i < range.end(); i++) {
            double min_dist = std::numeric_limits<double>::max();
            uint8_t cluster_id = 0;
            const Point& point = points.at(i);
            for (size_t j = 0; j < k; j++) {
              const Point& centroid = m_cluster_centers[j];
              double dist = point.dist(centroid);
              if (dist < min_dist) {
                min_dist = dist;
                cluster_id = j;
            ...
            if (m_cluster[i] != cluster_id) {
              m_cluster[i] = cluster_id;
              changes++;
            }
...
size_t KMeansParallel::compare(const std::vector<uint8_t>& first,
  ...
  return tbb::parallel_reduce(
      tbb::blocked_range<size_t>(0, first.size()), 0,
      [&](const tbb::blocked_range<size_t>& range, size_t good) {
        for (size_t i = range.begin(); i < range.end(); i++) {
